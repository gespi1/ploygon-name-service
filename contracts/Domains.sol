...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("XQC Name Service", "XNS") {
  ...
  function register(string calldata name) public payable{
    ...
    // check if value matches or is greater than price
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the XQC name service", "image": "data:image/svg+xml;base64,',
  ...
  // return owner address of domain name
  function getAddress(string calldata name) public view returns (address) {
    return domains[name];
  }

  //verify caller owns domain name and create record
  ...
  function price(string calldata name) public pure returns (uint) {
    ...
    uint xp = 17; // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals)
    require(len > 0);
  ...
  function valid(string calldata name) public pure returns(bool) {
    return StringUtils.strlen(name) >= 1 && StringUtils.strlen(name) <= 12;
  }
